 ...
 * Put your tests in "tests/pa2_tests" folder in your Eclipse workspace 
 * directory
 * If you preface your error messages / exceptions with ERROR or *** then they 
 * will be displayed if they appear during processing
 */
...
public class Checkpoint2 {
    ...
    public static void main(String[] args) throws IOException, 
        ...
        for (File x : testDir.listFiles()) {
            ...
            if (x.getName().indexOf("pass") != -1) {
                if (returnCode == 0) {
                    String actualAST = getAST(new FileInputStream(x.getPath() 
                        + ".out"));
                    if (actualAST.equals(ast))
                        System.out.println(x.getName() + 
                        " parsed successfully and has a correct AST!");
                    else {
                        System.err.println(x.getName() + 
                        " parsed successfully but has an incorrect AST!");
                        failures++;
                    }
                }
                else {
    ...
    private static ReturnInfo runTest(File x) throws IOException, 
        InterruptedException {
        ProcessBuilder pb = new ProcessBuilder("java", "miniJava.Compiler", 
            x.getPath()).directory(new File(
